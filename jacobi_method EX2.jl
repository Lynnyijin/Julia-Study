...
function jacobi_mpi_with_tol(n,tol,comm)
    u, u_new = init(n,comm)
    load = length(u)-2
    ...
    while true
        ...
        MPI.Waitall(reqs)
        u_new[2] = 0.5 * (u[1] + u[3])  
        u_new[load + 1] = 0.5 * (u[load] + u[load + 2]) 

        diff = max(diff, abs(u_new[2] - u[2]))
        diff = max(diff, abs(u_new[load + 1] - u[load + 1]))
        global_diff = MPI.Allreduce(diff, MPI.MAX, comm)
...
u = jacobi_mpi_with_tol(n,tol,comm)
